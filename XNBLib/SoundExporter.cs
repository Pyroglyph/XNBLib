...
namespace XNBLib
{
    ...
    {
        public static Result ConvertToWav(string inFile, string outFile = "")
        {
            if (outFile == "") outFile = Path.ChangeExtension(inFile, ".wav");
            ...
            using (Stream fs = File.OpenRead(inFile))
            {
                using (var br = new BinaryReader(fs))
                {
                    ...
                    if (fileLength != fs.Length)
                        return new Result(Result.Types.FileLengthMismatch, (fileLength + " - should be " + fs.Length));
            ...
            using (Stream fs = File.Create(outFile))
            {
